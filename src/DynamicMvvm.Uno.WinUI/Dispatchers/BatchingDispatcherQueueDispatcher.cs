...
using Chinook.DynamicMvvm.Extensions;
...
{
	/// <summary>
	/// This implementation of <see cref="IDispatcher"/> batches its operations to reduce the call count of <see cref="DispatcherQueueExtensions.RunAsync"/>.<br/>
	...
	/// After that short delay, all accumulated actions are executed within the same <see cref="DispatcherQueueExtensions.RunAsync"/>.
	/// </summary>
	public class BatchingDispatcherQueueDispatcher : IDispatcher
	{
		...
		public async Task ExecuteOnDispatcher(CancellationToken ct, Action action)
		{
			...
			await _dispatcherQueue.RunAsync(DispatcherQueuePriority.High, () =>
			{
